...
function FloatingCard({ friend, index, total }: { friend: typeof FRIENDS[0]; index: number; total: number }) {
  ...
  const badgeColors: Record<string, { bg: string; text: string; border: string }> = {
    shipped: { bg: "rgba(168,85,247,0.12)", text: "#a855f7", border: "rgba(168,85,247,0.25)" },
    wip: { bg: "rgba(251,191,36,0.1)", text: "#fbbf24", border: "rgba(251,191,36,0.2)" },
    live: { bg: "linear-gradient(135deg,#00ff88,#00ccff)", text: "#000", border: "transparent" },
  }
    ...
    >
      <div
        ...
        style={{
          ...
          borderColor: friend.type === "live" ? "rgba(0,255,136,0.15)" : "rgba(255,255,255,0.06)",
          backdropFilter: "blur(20px)",
          boxShadow: friend.type === "live"
            ? "0 8px 40px rgba(0,255,136,0.06), 0 0 0 1px rgba(0,255,136,0.05)"
      ...
      >
        ...
        <div className="flex items-center gap-2.5 px-4 pt-3.5 pb-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[13px] font-semibold text-[#f0f0f0]" style={{ fontFamily: "var(--font-display-family)" }}>
              {friend.name}
            </div>
        ...
        {/* Preview area */}
        <div className="mx-3 mb-2 rounded-xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.05)" }}>
          <div
            className="h-[120px] flex items-center justify-center"
            style={{
              background: `linear-gradient(135deg, ${
                friend.type === "shipped" ? "#0f172a, #1e1b4b"
                : friend.type === "live" ? "#0f172a, #1e1b4b"
                : "#0c0015, #1a0030"
              })`,
            }}
          >
            <div className="text-center">
              ...
              <div className="text-[10px] text-[#64748b] mt-1 px-4">{friend.desc}</div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5 px-4 pb-3">
          {REACTIONS.slice(0, 2 + (index % 2)).map((r, i) => (
            ...
            >
              <span>{r}</span>
              <span className="text-[10px] text-[#3a3a3a]">{Math.floor(Math.random() * 5) + 1}</span>
            </div>
          ))}
        </div>
      </div>
...
function PresenceRow() {
  return (
    <div className="flex items-center gap-1">
      ...
      <span className="text-[11px] text-[#3a3a3a] ml-1.5 font-medium">
        <span className="text-[#00ff88]">3</span> building
      </span>
    </div>
  )
}
    ...
    <div className="relative min-h-screen overflow-hidden" style={{ background: "#050505" }}>
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <div className="flex-1 flex flex-col justify-center px-6 sm:px-10 lg:px-16 max-w-[640px] pb-20">
          ...
          >
            ...
            />
            <span className="text-[13px] font-medium tracking-wide" style={{ color: "#5a5a5a" }}>
          ...
          >
            <Link
              ...
              style={{
                background: "linear-gradient(135deg, #00ff88, #00ccff)",
                color: "#000",
                boxShadow: "0 4px 24px rgba(0,255,136,0.2), 0 0 0 1px rgba(0,255,136,0.1)",
              }}
            >
      ...
      <style jsx>{`
        ...
        @keyframes ring-breathe {
          ...
          50% { box-shadow: 0 0 0 3px rgba(0, 255, 136, 0); }
        }
